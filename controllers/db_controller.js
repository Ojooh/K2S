...
module.exports.updateTaskStatus = (id, status, d_done) => {
    let query;
    if (d_done != '') {
        query = "UPDATE notify SET `is_complete` = '" + status + "', date_done = '" + d_done + "' WHERE id = '" + id + "';";
    } else {
        query = "UPDATE notify SET `is_complete` = '" + status + "' WHERE id = '" + id + "';";
    }
    return new Promise((resolve, reject) => {
        con.query(query, (err, result) => {
            if (err) {
...
//SPONSOR q-wallet update
module.exports.insertEwalletQ = (amount, qk, don, owner, name) => {
    const query = "INSERT INTO wallet (`amount`, `quick`, `donate`, `owner`, `wallet_name`, `auto`, `active`) VALUES ('" + amount + "', '" + qk + "','" + don + "', '" + owner + "', '" + name + "' , '0', '1'); ";
    console.log(query)
    return new Promise((resolve, reject) => {
        con.query(query, (err, result) => {
            if (err) {
...
module.exports.updateSponsorKids = (value, user) => {
